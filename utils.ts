...
/**
 * Clean phone and return full international number string
 * Example input: "+91 98635-98421"  -> "919863598421"
 */
export const cleanPhoneNumber = (phone: string): string => {
  if (!phone) return '';
  // keep only digits
  const digits = phone.replace(/\D/g, '');
  // If it already includes country code (length > 10) return as is
  if (digits.length > 10) return digits;
  // If only local 10‑digit Indian number, prepend 91
 ...
 * useWeb = true  -> WhatsApp Web (desktop)
 * useWeb = false -> whatsapp:// (mobile / APK - matches provided Java interceptor)
 */
export const getWhatsAppUrl = (
  ...
  const finalPhone = cleanPhoneNumber(phone); // already includes country code
  ...
  // Mobile / APK - Using whatsapp:// protocol to match the Android WebView interceptor provided
